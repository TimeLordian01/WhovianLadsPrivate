	...
	public void render(CoralConsoleTile tile, float v, MatrixStack matrixStack, IVertexBuilder iVertexBuilder, int i, int i1, float v1, float v2, float v3, float v4) {
       ...
       tile.getSubsystem(StabilizerSubsystem.class).ifPresent(sys ->{
		...
		tile.getControl(ThrottleControl.class).ifPresent(throttleControl -> {
			this.SmallLever2.rotateAngleX = -0.1F + throttleControl.getAmount() * 60;
		...
		tile.getControl(IncModControl.class).ifPresent(incModControl -> {
			this.SmallLever.rotateAngleX = (float) Math.toRadians(((incModControl.index / (float)IncModControl.COORD_MODS.length) * -180f) - 20f);
		});
