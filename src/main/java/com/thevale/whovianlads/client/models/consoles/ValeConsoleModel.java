package com.thevale.whovianlads.client.models.consoles;// Made with Blockbench 3.7.5
...
public class ValeConsoleModel extends EntityModel<Entity> implements TileModel<ValeConsoleTile> {
	...
	public void render(ValeConsoleTile tile, float v, MatrixStack matrixStack, IVertexBuilder iVertexBuilder, int i, int i1, float v1, float v2, float v3, float v4) {
		...
		tile.getControl(net.tardis.mod.controls.RandomiserControl.class).ifPresent(randomiserControl -> {
			this.RandomiserControl.rotateAngleY = (float) Math.toRadians(randomiserControl.getAnimationProgress() * 720.0F );
		});


		tile.getControl(HandbrakeControl.class).ifPresent(handbrakeControl -> {
		...
		tile.getControl(RefuelerControl.class).ifPresent(refuelerControl -> {
		...
		tile.getControl(ThrottleControl.class).ifPresent(throttleControl -> {
			this.throttle.rotationPointZ = -0.1F + throttleControl.getAmount() * 0.1F;
			this.throttle.rotationPointY = 0.05F + throttleControl.getAmount() * -0.05F;
		...
		tile.getControl(IncModControl.class).ifPresent(incModControl -> {
			this.Switch.rotateAngleX = (float) Math.toRadians(((incModControl.index / (float)IncModControl.COORD_MODS.length) * 100.0F) - 50.0F);
		});


		tile.getControl(XControl.class);
		//this.x.offsetY = x.getAnimationTicks() == 0 ? 0 : 0.006F;

		tile.getControl(YControl.class);
