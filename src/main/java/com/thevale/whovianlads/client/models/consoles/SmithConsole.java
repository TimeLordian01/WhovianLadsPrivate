...
public class SmithConsole extends EntityModel<Entity> implements TileModel<SmithConsoleTile> {
	...
	public void render(SmithConsoleTile tile, float v, MatrixStack matrixStack, IVertexBuilder iVertexBuilder, int i, int i1, float v1, float v2, float v3, float v4) {
		...
		tile.getControl(LandingTypeControl.class);
		...
		tile.getControl(ThrottleControl.class).ifPresent(throttleControl -> {
			this.Handle.rotateAngleX = 1F + throttleControl.getAmount() * 60;
		...
		tile.getControl(IncModControl.class).ifPresent(incModControl -> {
			this.Handle2.rotateAngleZ = (float) Math.toRadians(((incModControl.index / (float)IncModControl.COORD_MODS.length) * -180f) - 20f);
		});
